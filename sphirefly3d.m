...
video.Width = REPLACEME;
video.Height = REPLACEME;
video.FrameRate = REPLACEME;

% Set distance parameters manually:
cyber2world = [REPLACEME, 0.6]; % [horizontal distance, vertical distance] in meters.

sff.prm = setprm(cyber2world, video);

% Read in data
sff.gp1.cln.xyt = readmatrix("cam1_xyt.csv");
sff.gp2.cln.xyt = readmatrix("cam2_xyt.csv");

% Fix a bunch of flags
sff.prm.flag.trk = 1; %tracking done
sff.prm.flag.cln = 1; %cleaning done
sff.prm.flag.clb = 0; %calibrate
sff.prm.flag.trg = 0; %triangulate
sff.prm.flag.stk = 0; %straks
sff.prm.flag.trj= 0;  %trajectorize
